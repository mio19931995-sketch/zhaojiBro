...
function Inspector({ item, close, notify, refresh, navigate }: Shared & { item?: Item; close: () => void }) {
  ...
  useEffect(() => {
    ...
    if (item && !dirty) Promise.all([api<Item>(`/items/${item.id}`), readDraft<{ text: string; segments: Segment[]; tab: string }>(item.id)]).then(([d, saved]) => {
      if (!ignore) { setDetail(d); setDraft(saved?.text ?? d.transcript); setSegments(saved?.segments ?? d.segments); if (saved) { setTab(saved.tab); setDirty(true); } }
    }).catch(e => notify(e.message, true));
    return () => { ignore = true; };
  }, [item?.id, item?.updated_at, dirty, notify]);
  async function save() {
    if (!detail) return; setSaving(true);
    try { await api(`/items/${detail.id}`, 'PATCH', tab === 'timeline' ? { segments } : { transcript: draft }); await clearDraft(detail.id); setDirty(false); await refresh(); notify('文稿已保存'); } catch (e) { notify((e as Error).message, true); } finally { setSaving(false); }
  }
  ...
  async function discard() { if (detail) { await clearDraft(detail.id); setDirty(false); } }
  ...
  return <aside className="inspector"><div className="inspector-heading"><h2>文稿预览</h2><Button title="关闭预览" onClick={close}><X size={16}/></Button></div>{!detail ? <Empty icon={<FileText size={32}/>} title="在素材库打开任意一条" text="这里会显示完整的文稿、来源和试听。"/> : <>
    ...
    {detail.metadata.cover_path && <img className="inspector-cover" src={`/api/items/${detail.id}/cover`} alt="作品封面" onDoubleClick={()=>reveal(detail.id,'cover').catch(e=>notify(e.message,true))}/>}
    {detail.media_path && <div className="media-player">{video ? <video ref={el => { player.current = el; }} controls preload="metadata" src={`/api/items/${detail.id}/media`}/> : <audio ref={el => { player.current = el; }} controls preload="metadata" src={`/api/items/${detail.id}/media`}/>}</div>}
