...
declare global {
  interface Window { desktop?: { copyText?: (text: string) => Promise<void>; openData: () => Promise<void>; showItem: (id: string,kind?:string) => Promise<void>; chooseFolder: () => Promise<string>; openExternal: (url: string) => Promise<void>; platform:(options:{url:string;limit?:number;collectionId?:string;loginOnly?:boolean})=>Promise<{entries:any[];complete:boolean;error:string}>; stopPlatform:()=>Promise<void>; setFullscreen:(value:boolean)=>Promise<boolean>; onFullscreenChange:(callback:(value:boolean)=>void)=>()=>void } }
}
